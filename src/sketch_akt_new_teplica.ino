...
String s_1("дождь");
 String s_2("сухо");
 String s_3("почва высохла");
 String s_4("почва влажная");
 String s_5("почва мокрая");
...
void build(gh::Builder& b) {
  ...
  {
  gh::Row r(b);
  if (b.Tabs(&tab).label(F("ТАЙМЕР")).text("пуск;стоп").click()) b.refresh(); // обновить по клику
  switch (tab) {
    case 0:
         Serial.println("пуск");
         s=1;
   ...
   if(b.Select(&sel).label(F("ВЫБОР ПОЛИВА")).text("капель;насос;капель_насос").click()) b.refresh();
   switch (sel) {
    case 0:
...
void krusha(){
  if(g==100){
     val=s_1;
    hub.sendUpdate("disp_1", val);
  }
}
void pochva(){
  if(g==100){
     val_1=s_3;
    hub.sendUpdate("disp_2", val_1);
  }
}
void loop() {
  // =================== ТИКЕР ===================
